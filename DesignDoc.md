...
NewsLens は、「偏りのない記事」や「完全に中立な要約」が
理論的に存在するとは考えない。
...
NewsLens の構造化出力は、答えを与えるためではなく、「どこが分からないのか」「何がまだ決まっていないのか」にユーザー自身が気づくための装置である。
...
ユーザーが自分なりの判断に到達するために必要な視点・論点・背景（解釈の軸）を補助的に提示する ことを唯一の目的とする。
